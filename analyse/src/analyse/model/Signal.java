...
public class Signal implements IResource {

	private String name;
	private String oldName;
	private Subject subject;
	
	public Signal(String name, Subject subject) {
		this.name = name;
		oldName = name;
	...
	public boolean delete() {
		...
		if(!succeed) Log.logErrorMessage("Impossible to delete signal " + getLocalPath());
	...
	public void rename(String newName) {
		IMathEngine mathEngine = MathEngineFactory.getInstance().getMathEngine();
		if(!mathEngine.renameChannel(getParent().getLocalPath(), name, newName)) Log.logErrorMessage("Impossible to rename Signal in Math Engine : " + getLocalPath() + " (old name : " + oldName + " new name : " + name +") !");
		else {
			if(!oldName.equals(name)) oldName = name;
			name = newName;
			((Subject)getParent()).setModified(true);
		}
	}
