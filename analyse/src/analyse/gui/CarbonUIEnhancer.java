...
public class CarbonUIEnhancer {
	...
	public CarbonUIEnhancer(String aboutTitle) {
		...
		String about = "About " + aboutTitle;
		if (AnalysePreferences.getPreferenceStore()
				.getString(AnalysePreferences.LANGUAGE)
				.equals(AnalysePreferences.FRENCH_LANGUAGE))
			about = "\u00c0 propos de " + aboutTitle;
		fgAboutActionName = about;
	}

	/**
	 * See Apple Technical Q&A 1079
	 * (http://developer.apple.com/qa/qa2001/qa1079.html)
	 */
	@SuppressWarnings("rawtypes")
	public void hookApplicationMenu(Display display,
			final Listener quitListener, final IAction aboutAction,
			final IAction preferencesAction) {
		// Callback target
		Object target = new Object() {
			@SuppressWarnings({ "unused" })
			int commandProc(int nextHandler, int theEvent, int userData) {
				int osActualEventKind = (Integer) invoke(osCls, "GetEventKind",
						new Class[] { int.class }, theEvent);
				int osEventProcessCommand = (Integer) fieldValue(osCls,
				...
				if (osActualEventKind == osEventProcessCommand) {
					...
					Class[] pt = new Class[] { int.class, int.class, int.class,
							int[].class, int.class, int[].class, hiCmdCls };
					invoke(osCls, "GetEventParameter", pt, theEvent,
							fieldValue(osCls, "kEventParamDirectObject"),
							fieldValue(osCls, "typeHICommand"), null,
							fieldValue(command, "sizeof"), null, command);
		...
		final Object commandCallback = newInstance(
				"org.eclipse.swt.internal.Callback", pt, target, "commandProc",
				3);
		...
		int[] mask = new int[] {
				(Integer) fieldValue(osCls, "kEventClassCommand"),
				(Integer) fieldValue(osCls, "kEventProcessCommand") };
		pt = new Class[] { int.class, int.class, int.class, int[].class,
				int.class, int[].class };

		int appEventTarget = (Integer) invoke(osCls,
				"GetApplicationEventTarget");
		invoke(osCls, "InstallEventHandler", pt, appEventTarget, commandProc,
				mask.length / 2, mask, 0, null);

		// create About Eclipse menu command
		int[] outMenu = new int[1];
		short[] outIndex = new short[1];
		// GetIndMenuItemWithCommandID(int mHandle, int commandId, int index,
		// int[] outMenu, short[] outIndex);
		int indMenuItem = (Integer) invoke(osCls,
				"GetIndMenuItemWithCommandID", new Class[] { int.class,
						int.class, int.class, int[].class, short[].class }, 0,
				kHICommandPreferences, 1, outMenu, outIndex);
		...
		if (indMenuItem == osNoErr && outMenu[0] != 0) {
			...
			int str = (Integer) invoke(osCls, "CFStringCreateWithCharacters",
			...
			// kHICommandAbout);
			invoke(osCls, "InsertMenuItemTextWithCFString", new Class[] {
					int.class, int.class, short.class, int.class, int.class },
			...
			invoke(osCls, "InsertMenuItemTextWithCFString", new Class[] {
					int.class, int.class, short.class, int.class, int.class },
					menu, 0, (short) 1,
					fieldValue(osCls, "kMenuItemAttrSeparator"), 0);

			// enable pref menu
			// OS.EnableMenuCommand(menu, kHICommandPreferences);
			invoke(osCls, "EnableMenuCommand", new Class[] { int.class,
					int.class }, menu, kHICommandPreferences);
			// disable services menu
			// OS.DisableMenuCommand(menu, kHICommandServices);
			invoke(osCls, "DisableMenuCommand", new Class[] { int.class,
					int.class }, menu, kHICommandServices);
	...
	private int runAction(IAction action) {
		try {
			...
			System.out.println("Error while executing Apple menu action: "
					+ e.getMessage());
		}
		return (Integer) fieldValue(osCls, "eventNotHandledErr");
	}
	...
	@SuppressWarnings("rawtypes")
	private Object newInstance(String classname, Class[] paramTypes,
			Object... arguments) {
		try {
			Class cls = classForName(classname);
	...
	private Object invoke(@SuppressWarnings("rawtypes") Class cls, String methodName) {
		return invoke(cls, methodName, null, (Object[]) null);
	}

	private Object invoke(@SuppressWarnings("rawtypes") Class cls, String methodName, @SuppressWarnings("rawtypes") Class[] paramTypes,
			Object... arguments) {
		try {
	...
	private Object invoke(Object obj, String methodName, @SuppressWarnings("rawtypes") Class[] paramTypes,
			Object... arguments) {
		try {
	...
	private Object fieldValue(@SuppressWarnings("rawtypes") Class cls, String fieldName) {
		try {
