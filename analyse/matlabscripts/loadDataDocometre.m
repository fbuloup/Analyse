function loadDataDocometre(experimentName,subjectName, dataFile) %, options)
      
x = findstr(dataFile,'.sau,');
if isempty(x) == 0
	loadDataDocometreSAU(experimentName, subjectName, dataFile);
else
        ...
        if nbChannelsInTrial > 0            
            ...
            for currentNumChannel = 1:nbChannelsInTrial     
                ...
                if nbData >= 0
                    ...
                    fullSignalName = [experimentName '.' subjectName '.' signalName ];
    ...
    for currentNumTrial = 1:nbTrials
        ...
        for currentNumChannel = 1:nbChannelsInTrial
            ...
            if nbTrialsWithChannels == 1
                ...
                cmd = strcat('signalExist = isfield(',cmd,');');
                eval(cat(2,cmd{:}));
                if(signalExist == 0)
                    eval([fullSignalName, ' = signal;']);
