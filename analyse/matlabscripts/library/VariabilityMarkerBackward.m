...
%It computes mean and standard deviation between two markers. Going backward
%from a third specified marker, it checks when amplitude value goes in or out
%variability (mean plus or minus a percent of standard deviation).function VariabilityMarkerBackward (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, AllowedVariability, MarkMethod)
%beginAnalyseHeader
%GUIFunctionName = Variability Marker Backward
%MatlabFunctionName = VariabilityMarkerBackward
%ShortDescritpion = <p>his function marks the point where signal goes out or in a specified variability.</p>
%LongDescription = <p>This function marks the point where signal goes in or out a specified variability. It computes mean and standard deviation between two markers. Going backward from a third specified marker, it checks when amplitude value goes in or out variability (mean plus or minus a percent of standard deviation).</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 3
%FieldsUsedNumber = 0
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		if(fromIndex > toIndex)
			...
			if( strcmp(MarkMethod, 'Enter in variability') == 1)
				while (~((currentValue < valueMax) && (currentValue > valueMin)) && (i >= 1))
					i=i-1;
					if(i >= 1) currentValue = InputSignal1.Values(TrialNumber,i);
				end
			else
				while ((currentValue < valueMax) && (currentValue > valueMin) && (i >= 1)
					i=i-1;
