%This function finds an amplitude, specified as parameter, starting from the first input marker and going forward to the other marker.function FindAmplitudeForward (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Amplitude)
%beginAnalyseHeader
%GUIFunctionName = Find Amplitude Forward
%MatlabFunctionName = FindAmplitudeForward
%ShortDescritpion = <p>This function finds an amplitude, specified as parameter, starting from the first input marker and going forward to the other marker.</p>
%LongDescription = <p>This function finds an amplitude, specified as parameter, starting from the first input marker and going forward to the other marker.</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
%param1Component = 1
%param1Label = Amplitude
%param1ToolTip = Amplitude to find
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		i = fromIndex + 1;
		previousValue =  InputSignal1.Values(TrialNumber, fromIndex);
		found = 0;
		while(i <= toIndex)
			...
			if(found == 1)
				...
				if(dCurrent > dPrevious) 
					OutputMarker1.Values = [OutputMarker1.Values; TrialNumber (i-2)/InputSignal1.SampleFrequency previousValue];
					break;
				else
					OutputMarker1.Values = [OutputMarker1.Values; TrialNumber (i-1)/InputSignal1.SampleFrequency currentValue];
					break;
				end
			end
