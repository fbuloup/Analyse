%Copy a signal between two time markers 
%into a new signalfunction CopySignalPart (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = CopySignalPart
%MatlabFunctionName = CopySignalPart
%ShortDescritpion = <p>Create a new signal from a part of another one specified by two markers</p>
%LongDescription = <p>Create a new signal from a part of another one specified by two markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
%beginFunction
[A, idxSorted] = sort(InputMarker1.Values(:,1));
trialNumberMax = max(A);
InputMarker1.Values = InputMarker1.Values(idxSorted, :);

[A, idxSorted] = sort(InputMarker2.Values(:,1));
InputMarker2.Values = InputMarker2.Values(idxSorted, :);

nbSamples = max(InputMarker2.Values(:,2) - InputMarker1.Values(:,2))*InputSignal1.SampleFrequency + 1;

OutputSignal1.Values = NaN.*ones(trialNumberMax,nbSamples);
OutputSignal1.NbSamples = round( nbSamples.*ones(1,trialNumberMax)' );
OutputSignal1.EndCut = OutputSignal1.NbSamples.*ones(1,trialNumberMax)';
OutputSignal1.FrontCut = ones(1,trialNumberMax)';
%Loop over trials
for TrialNumber = TrialsList
	fromIndex = round(InputMarker1.Values(TrialNumber,2)*InputSignal1.SampleFrequency+1);
	toIndex = round(InputMarker2.Values(TrialNumber,2)*InputSignal1.SampleFrequency+1);
	
	idxTimeStart = find(InputMarker1.Values(:,1) == TrialNumber)
	if( length(idxTimeStart) ~= 1) error('Wrong trial number'); end;
	
	idxTimeEnd = find(InputMarker2.Values(:,1) == TrialNumber)
	if( length(idxTimeEnd) ~= 1) error('Wrong trial number'); end;
	
	idxStart = round(InputMarker1.Values(idxTimeStart,2) * InputSignal1.SampleFrequency)
	idxEnd = round(InputMarker2.Values(idxTimeEnd,2) * InputSignal1.SampleFrequency)

	OutputSignal1.Values(TrialNumber,1:idxEnd - idxStart + 1) = InputSignal1.Values(TrialNumber,idxStart:idxEnd);
	
	
	OutputSignal1.NbSamples(TrialNumber) = round(idxEnd - idxStart + 1);
	OutputSignal1.EndCut(TrialNumber) = round(idxEnd - idxStart + 1);
