%This fonction creates new markers in the input 
%signal comming from time values of the input 
%markers
function TransfertMarker (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Transfert Marker
%MatlabFunctionName = TransfertMarker
%ShortDescritpion = <p>Transfert marker from a signal to another one</p>
%LongDescription = <p>This fonction creates new markers in the input signal comming from time values of the input markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 1
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(index))
		...
		for i=1:length
			indice = round(InputMarker1.Values(index(i),2)*InputSignal1.SampleFrequency + 1);	
			OutputMarker1.Values=[OutputMarker1.Values; TrialNumber InputMarker1.Values(index(i),2) InputSignal1.Values(TrialNumber,indice)];
		end
