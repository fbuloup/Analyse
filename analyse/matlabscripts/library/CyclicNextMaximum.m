%This function finds next maximum after a specified marker and before second one
function CyclicNextMaximum (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Next Maximum
%MatlabFunctionName = CyclicNextMaximum
%ShortDescritpion = <p>Next Maximum between two markers</p>
%LongDescription = <p>This function finds next maximum after a specified marker and before a second one</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		nbFromTrialIndices = size(fromTrialIndices);
		nbFromTrialIndices = nbFromTrialIndices(1);
		
		nbToTrialIndices = size(toTrialIndices);
		nbToTrialIndices = nbToTrialIndices(1);
		
		
		m=1;
		for n = 1:nbFromTrialIndices
		
			if(m<nbToTrialIndices)
				 m = m + 1;
			end
		
			fromIndex = round(InputMarker1.Values(fromTrialIndices(n), 2)*InputSignal1.SampleFrequency+1);
			toIndex =round( InputMarker2.Values(toTrialIndices(m), 2)*InputSignal1.SampleFrequency+1);
		
			%Compute derivative between  first time index and last time
			dSignal = diff(InputSignal1.Values(TrialNumber,fromIndex:toIndex));
			
			%Get array indices where derivative sign computed previously is equals  to -2
			maxIndices = find(diff(sign(dSignal)) == -2);
			
			if(~isempty(maxIndices))
				%Compute real index
				realIndex = fromIndex + maxIndices(1);
				%Create marker
				OutputMarker1.Values=[OutputMarker1.Values; TrialNumber (realIndex-1)/InputSignal1.SampleFrequency InputSignal1.Values(TrialNumber,realIndex)];
			end
