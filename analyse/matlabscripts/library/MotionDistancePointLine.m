%This function computes the perpendicular distance between every motion's points and the
%line that joins the start and the end of this motion. It takes two signals
%as inputs and calculates this distance at any point of the trajectory.
%This trajectory is formed from the two input signals.
function MotionDistancePointLine (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Motion Distance From Point To Line
%MatlabFunctionName = MotionDistancePointLine
%ShortDescritpion = <p>This function computes distance from every motion's points to the line that joins the start and end of this motion</p>
%LongDescription = <p>This function computes distance from every motion's points to the  line that joins the start and end of this motion. It takes two signals  as inputs and calculates this distance at any point of the trajectory.  This trajectory is formed from the two input signals.</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		%Compute straight line parameters from start to end positions : slope and intercept
		slope = ( InputSignal2.Values(TrialNumber,toIndex)-InputSignal2.Values(TrialNumber,fromIndex) )/( InputSignal1.Values(TrialNumber,toIndex)-InputSignal1.Values(TrialNumber,fromIndex) );
		intercept = InputSignal2.Values(TrialNumber,fromIndex) - InputSignal1.Values(TrialNumber,fromIndex)*slope;
		
		%Compute distances from formula : d = | y - slope*x - intercept | / sqrt( slope² + 1 )
		for i=fromIndex:toIndex
			OutputSignal1.Values(TrialNumber,i) = abs(  InputSignal2.Values(TrialNumber,i) -slope*InputSignal1.Values(TrialNumber,i) - intercept ) / sqrt( slope^2 + 1 );
		end
