%This function computes motion distance in 3D. It takes three signals
%as inputs and calculates the distance along the curve between the
%origin and any point of the trajectory. This trajectory is 
%formed from the three input signals.
function MotionDistance3D (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Motion distance 3D
%MatlabFunctionName = MotionDistance3D
%ShortDescritpion = <p>This function computes motion distancein 3D</p>
%LongDescription = <p>This function computes motion distance in 3D. It takes three signals as inputs and calculates the distance along the curve between the origin and any point of the trajectory. This trajectory is  formed from the three input signals.</p>
%SignalsUsedNumber = 3
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))		
		...
		OutputSignal1.Values(TrialNumber, fromIndex)=0;
	                ...
	                dx2  =( diff( InputSignal1.Values(TrialNumber, fromIndex:toIndex) ) ).^2;
	                dy2  =( diff( InputSignal2.Values(TrialNumber, fromIndex:toIndex) ) ).^2;
	                dz2  =( diff( InputSignal3.Values(TrialNumber, fromIndex:toIndex) ) ).^2;
	                OutputSignal1.Values(TrialNumber,fromIndex + 1:toIndex)= sqrt( dx2 + dy2 +dz2 );
			
		OutputSignal1.Values(TrialNumber,fromIndex:toIndex) = cumsum(OutputSignal1.Values(TrialNumber,fromIndex:toIndex)')';	
	end
