%This function computes motion distance. It takes two signals
...
%formed from the two input signals.
function MotionDistance (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Motion distance
%MatlabFunctionName = MotionDistance
%ShortDescritpion = <p>This function computes motion distance</p>
%LongDescription = <p>This function computes motion distance. It takes two signals as inputs and calculates the distance along the curve between the origin and any point of the trajectory. This trajectory is  formed from the two input signals.</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		dx2  =( diff( InputSignal1.Values(TrialNumber, fromIndex:toIndex) ) ).^2;
	                ...
	                OutputSignal1.Values(TrialNumber,fromIndex + 1:toIndex)= sqrt( dx2 + dy2);
