...
function Derivative (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Derivative
%MatlabFunctionName = Derivative
%ShortDescritpion = <p>Derivative</p>
%LongDescription = <p>Derivative of the signal</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 0
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	OutputSignal1.Values(TrialNumber,:)=[diff(InputSignal1.Values(TrialNumber,:)) 0]*InputSignal1.SampleFrequency;
	
	%deriv=[];
	%for i=1:2:L-2
	%	amp(i)=InputSignal1.Values(TrialNumber,i+2)-InputSignal1.Values(TrialNumber,i);
	%	deriv=[deriv amp(i)*InputSignal1.SampleFrequency amp(i)*InputSignal1.SampleFrequency];
	%end
	%deriv(L)=deriv(L-1);
	
	%OutputSignal1.Values(TrialNumber,:)=deriv;
