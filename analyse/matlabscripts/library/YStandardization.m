%Enter your comments herefunction YStandardization (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Value)
%beginAnalyseHeader
%GUIFunctionName = YStandardization
%MatlabFunctionName = YStandardization
%ShortDescritpion = <p>Y axis standardization</p>
%LongDescription = <p>Y axis standardization</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 0
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	maxi=max(InputSignal1.Values(TrialNumber,:));
	mini=min(InputSignal1.Values(TrialNumber,:));
	
	if abs(maxi)>abs(mini)
		OutputSignal1.Values(TrialNumber,:)=InputSignal1.Values(TrialNumber,:)/maxi*Value;
	else
		OutputSignal1.Values(TrialNumber,:)=InputSignal1.Values(TrialNumber,:)/abs(mini)*Value;
	end
