...
function Truncation (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Frontc, Endc)
%beginAnalyseHeader
%GUIFunctionName = Truncation
%MatlabFunctionName = Truncation
%ShortDescritpion = <p>Signal truncation</p>
%LongDescription = <p>Signal truncation</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 0
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	i=floor(Frontc*InputSignal1.SampleFrequency);
	j=floor(Endc*InputSignal1.SampleFrequency);
	
	OutputSignal1.FrontCut(TrialNumber)=i;
	OutputSignal1.EndCut(TrialNumber)=j;
	
	OutputSignal1.Values=[OutputSignal1.Values; zeros(1,j-i)];
	
	for k=1:j-i
		OutputSignal1.Values(TrialNumber,k)=InputSignal1.Values(TrialNumber,i+k);
	end
