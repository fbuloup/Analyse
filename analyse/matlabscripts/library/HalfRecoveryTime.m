%Create a marker of the "half recovery time" th indicating the
%time required  for the signal to decrease to half the burst 
%amplitude value after a sudden signal burst. Therefore, 
%given two markers at times t1 and t2 (with th>t2>t1) 
%standing respectively for a minimal and a maximal 
%amplitude a1 and a2, th is the time value of the first point 
%following t2 where the signal amplitude goes over a =(a2-a1)/2. function HalfRecoveryTime (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Half Recovery Time
%MatlabFunctionName = HalfRecoveryTime
%ShortDescritpion = <p>Create a marker of the "half recovery time" th indicating the time required  for the signal to decrease to half the burst amplitude value after a sudden signal burst</p>
%LongDescription = <p>Create a marker of the "half recovery time" th indicating the time required  for the signal to decrease to half the burst amplitude value after a sudden signal burst. Therefore, given two markers at times t1 and t2 (with th>t2>t1) standing respectively for a minimal and a maximal amplitude a1 and a2, th is the time value of the first point following t2 where the signal amplitude goes over (a2-a1)/2</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	index1 = find(InputMarker1.Values(:,1)==TrialNumber);
	index2 = find(InputMarker2.Values(:,1)==TrialNumber);
	L=length(InputSignal1.Values(TrialNumber,:));
	
	if(~isempty(index1) && ~isempty(index2))
		
		A0 = InputMarker1.Values(index1(1),3);
		A1 = InputMarker2.Values(index2(1),3);
		deltaA =   (A0 - A1)/2 ;
		
		i=round(InputMarker2.Values(index2(1),2)*InputSignal1.SampleFrequency+1)
		
		if(sign(deltaA) == 1)
			while ((InputSignal1.Values(TrialNumber,i) < A1 + deltaA) & (i<L))
				i=i+1;
			end
		end
		...
		if (i<L)
			i
			OutputMarker1.Values=[OutputMarker1.Values; TrialNumber (i-1)/InputSignal1.SampleFrequency InputSignal1.Values(TrialNumber,i)];
		end
