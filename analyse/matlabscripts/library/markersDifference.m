%Compute Time differences between two group of markers.
%There must be the same number of markers in groups.
%Differences will be placed in "Amplitude" values of the new markers group.
%New Markers "Time" values will be the same as first markers group input. 
%New marker group will be placed in signal input.function MarkersDifference (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = markers Difference
%MatlabFunctionName = MarkersDifference
%ShortDescritpion = <p>Compute Time difference between two group markers</p>
%LongDescription = <p>Compute Time difference between two group of markers. There must be the same number of markers in both groups. Differences will be placed in "Amplitude" values of the new markers group. New Markers "Time" values will be the same as first markers group input.</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	indices1 = find(InputMarker1.Values(:,1) == TrialNumber);
	indices2 = find(InputMarker2.Values(:,1) == TrialNumber);
	
	if(~isempty(indices1) && ~isempty(indices2))	
		%Get only time and amplitude values from current trial
		InputMarkerSubValues1 = InputMarker1.Values(indices1,2);
		InputMarkerSubValues2 = InputMarker2.Values(indices1,2);
		%Sort these values by ascending order on first column (time)
		sortedMark1 = sortrows(InputMarkerSubValues1,1);
		sortedMark2 = sortrows(InputMarkerSubValues2,1);
		%Loop to compute differences if same size
		if(length(sortedMark1) == length(sortedMark2))
			for i=1:length(sortedMark1)
				OutputMarker1.Values=[OutputMarker1.Values; TrialNumber, sortedMark1(i,1), sortedMark2(i,1)-sortedMark1(i,1)]; 
			end
		end
	end
end
