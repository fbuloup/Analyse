%This function computes motion direction. It takes two signals
%as inputs and calculates the direction along the curve between the
%origin and any point of the trajectory and the ahead direction.
%This trajectory is formed from the two input signals. First specified 
%signal is X values, second specified signal is Y values.function MotionDirection (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Degree)
%beginAnalyseHeader
%GUIFunctionName = Motion Direction
%MatlabFunctionName = MotionDirection
%ShortDescritpion = <p>This function computes motion direction</p>
%LongDescription = <p>This function computes motion direction. It takes two signals  as inputs and calculates the direction along the curve between the  origin and any point of the trajectory and the ahead direction.  This trajectory is formed from the two input signals. First specified signal is X values, second specified signal is Y values.</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
%param1MatlabType = integer
%param1Component = 2
%param1Label = Degree
%param1ToolTip = Compute results in degrees
%param1RegExp = (true|false)
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		x0 = InputSignal1.Values(TrialNumber,fromIndex);
		y0 = InputSignal2.Values(TrialNumber,fromIndex);
		OutputSignal1.Values(TrialNumber,fromIndex)=0;
		for i=1:toIndex
			if(i ~= fromIndex)
				dx = (InputSignal1.Values(TrialNumber,i) - x0);
				dy = (InputSignal2.Values(TrialNumber,i) - y0);
				...
				OutputSignal1.Values(TrialNumber,i)=theta;
			end
		end
	end
