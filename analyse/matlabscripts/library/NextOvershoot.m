%This function find the first point in the input sequence for which
%the amplitude value is greater than the amplitude value of
%the input marker plus a threshold that must be given as parameter.
function NextOvershoot (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Threshold)
%beginAnalyseHeader
%GUIFunctionName = Next Overshoot between two markers
%MatlabFunctionName = NextOvershoot
%ShortDescritpion = <p>Find next overshoot from marker</p>
%LongDescription = <p>This function find the first point in the input sequence for which  the amplitude value is greater than the amplitude value of  the input marker plus a threshold that must be given as parameter.  </p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
%param1ToolTip = Value that will be added to marker input level
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		ThresholdValue = InputMarker1.Values(fromTrialIndices(1),3) + Threshold;
		i=fromIndex
		
		if(sign(Threshold) == 1)
			while ((InputSignal1.Values(TrialNumber,i)<ThresholdValue)&(i<toIndex))
				i=i+1;
			end
		end
