%This function marks the point where signal goes in or out a specified variability.
%It computes mean and standard deviation between two markers. Going forward
%from a third specified marker, it checks when amplitude value goes in or out
%variability (mean plus or minus a percent of standard deviation).function VariabilityMakerForward (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, AllowedVariability, MarkMethod)
%beginAnalyseHeader
%GUIFunctionName = Variability Maker Forward
%MatlabFunctionName = VariabilityMakerForward
%ShortDescritpion = <p>This function marks the point where signal goes out or in a specified variability.</p>
%LongDescription = <p>This function marks the point where signal goes in or out a specified variability. It computes mean and standard deviation between two markers. Going forward from a third specified marker, it checks when amplitude value goes in or out variability (mean plus or minus a percent of standard deviation).</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 3
%FieldsUsedNumber = 0
%SignalsCreatedNumber = 0
%MarkersCreatedNumber = 1
%fieldsCreatedNumber = 0
%signalsModifiedNumber = 0
%param1MatlabType = float
%param1Component = 1
%param1Label = AllowedVariability
%param1ToolTip = Allowed variability in percent of standard deviation
%param1RegExp = (^[+]?\d*\.?\d*[1-9]+\d*([eE][-+]?[0-9]+)?$)|(^[+]?[1-9]+\d*\.\d*([eE][-+]?[0-9]+)?$)
%param1AvailableListValue = 
%param1DefaultValue = 100.0
%param1SignalsAvailable = false
%param1MarkersAvailable = false
%param1FieldsAvailable = false
%param2MatlabType = string
%param2Component = 3
%param2Label = MarkMethod
%param2ToolTip = Mark when entering or exiting from variability
%param2RegExp = ^(\w+\s*)*$
%param2AvailableListValue = Enter in variability:Exit from variability
%param2DefaultValue = Enter in variability
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		if(fromIndex < toIndex)
			startIndex = round( InputMarker3.Values(startTrialIndices(1), 2)*InputSignal1.SampleFrequency+1);
			L=length(InputSignal1.Values(TrialNumber,:));
			standardDeviation = std( InputSignal1.Values( TrialNumber, fromIndex:toIndex ));
			percent = AllowedVariability/100;
			average = mean(InputSignal1.Values(TrialNumber, fromIndex:toIndex));
			valueMax = average + percent * standardDeviation;
			valueMin = average - percent * standardDeviation;
			i = startIndex;
			currentValue = InputSignal1.Values(TrialNumber,i);
			if( strcmp(MarkMethod, 'Enter in variability') == 1)
				while (~((currentValue < valueMax) && (currentValue > valueMin)) && (i <= L))
					i = i+1;
					currentValue = InputSignal1.Values(TrialNumber,i);
				end
			else
				while ((currentValue < valueMax) && (currentValue > valueMin) && (i <= L))
					i = i+1;
					if(i <= L) currentValue = InputSignal1.Values(TrialNumber,i);
				                end
			                end
			end
			if(i <= L)
				OutputMarker1.Values=[OutputMarker1.Values; TrialNumber i/InputSignal1.SampleFrequency InputSignal1.Values(TrialNumber,i)];
			end
		end
