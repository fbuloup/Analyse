%This function computes total time spent at a particular value between two markers.
%This value has to be given as parameter with an other parameter
%called 'epsilon' expressed in precent. Current value is compared to the parameter value
%plus or minus epsilon.function TotalTimeAtValue (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Value, Epsilon)
%beginAnalyseHeader
%GUIFunctionName = Total Time At Value
%MatlabFunctionName = TotalTimeAtValue
%ShortDescritpion = <p>This function computes total time spent at a particular value between two markers</p>
%LongDescription = <p>This function computes total time spent at a particular value  between two markers.  This value has to be given as parameter with an other parameter  called 'epsilon' expressed in percent. Current value is compared to the parameter value  plus or minus epsilon.</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		values = InputSignal1.Values(TrialNumber,fromIndex:toIndex);
		Value1 = Value - abs(Value)*Epsilon/100;
		Value2 = Value + abs(Value)*Epsilon/100;
	
		nbValueIndices = length( find( (values < Value2) &  (values > Value1) ) );
		if(nbValueIndices == 0) 
			nbValueIndices = 1;
		end;
		OutputField1.Values(TrialNumber) = (nbValueIndices-1)/InputSignal1.SampleFrequency;
	end
end
%endFunction
%
