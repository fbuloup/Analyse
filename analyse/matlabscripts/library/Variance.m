%Variance between two markers
function Variance (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Variance
%MatlabFunctionName = Variance
%ShortDescritpion = <p>Variance between two markers</p>
%LongDescription = <p>Variance between two markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		sum=0;
		for i=1:L
			ecart(i)=InputSignal1.Values(TrialNumber,i)-mean(InputSignal1.Values(TrialNumber,fromIndex:toIndex));
			sum=sum+(ecart(i))^2;
		end
		
		OutputField1.Values(TrialNumber)=sum/L;
	end
