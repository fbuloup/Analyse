%Creates a marker at the absolute minimum of the signal between two markers
function Minimum (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Minimum
%MatlabFunctionName = Minimum
%ShortDescritpion = <p>Minimum between two markers</p>
%LongDescription = <p>Creates a marker at the absolute minimum of the signal between two markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		[minValue, minIndex] = min(InputSignal1.Values(TrialNumber,fromIndex:toIndex))
		OutputMarker1.Values=[OutputMarker1.Values; TrialNumber (fromIndex + minIndex-2)/InputSignal1.SampleFrequency minValue];
	end
end
%endFunction
%
