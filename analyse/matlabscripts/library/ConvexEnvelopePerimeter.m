%Convex envelope perimeter between two markersfunction ConvexEnvelopePerimeter (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Convex Envelope Perimeter
%MatlabFunctionName = ConvexEnvelopePerimeter
%ShortDescritpion = <p>Convex envelope perimeter between two markers</p>
%LongDescription = <p>Convex envelope perimeter between two markers</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		x=InputSignal1.Values(TrialNumber,fromIndex:toIndex);
		y=InputSignal2.Values(TrialNumber,fromIndex:toIndex);
		k=convhull(x,y);
		hull=[x(k);y(k)];
		
		perim=0;
		for i=1:length(hull)-1
			diff1=hull(1,i+1)-hull(1,i);
			diff2=hull(2,i+1)-hull(2,i);
			perim=perim+sqrt(diff1^2+diff2^2);
		end
		
		OutputField1.Values(TrialNumber)=perim;
