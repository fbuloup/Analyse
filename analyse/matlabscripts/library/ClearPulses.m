%This function supress pulses that have a too short duration.
%This duration must be given as parameter
function ClearPulses (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Duration)
%beginAnalyseHeader
%GUIFunctionName = Clear pulses
%MatlabFunctionName = ClearPulses
%ShortDescritpion = <p>This function supress pulses that have a too short duration.</p>
%LongDescription = <p>This function supress pulses that have a too short duration.  This duration must be given as parameter</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 0
%FieldsUsedNumber = 0
...
%param1Component = 1
%param1Label = Duration
%param1ToolTip = Max pulse duration (ms)
%param1RegExp = (^[+]?\d*\.?\d*[1-9]+\d*([eE][-+]?[0-9]+)?$)|(^[+]?[1-9]+\d*\.\d*([eE][-+]?[0-9]+)?$)
%param1AvailableListValue = 
%param1DefaultValue = 0.01
...
for TrialNumber = TrialsList
	...
	thresholdIndex = Duration * InputSignal1.SampleFrequency;
	frontEdgeIndex = 1;
	frontEdgeValue = InputSignal1.Values(TrialNumber,1);
	OutputSignal1.Values(TrialNumber,:)=InputSignal1.Values(TrialNumber,:);
	
	for i=2:length(InputSignal1.Values(TrialNumber,:))
		if InputSignal1.Values(TrialNumber,i) ~= InputSignal1.Values(TrialNumber,i-1)
			
			if (frontEdgeIndex > 0) & (i-frontEdgeIndex < thresholdIndex)
				OutputSignal1.Values(TrialNumber,frontEdgeIndex+1 : i-1) = frontEdgeValue;
			end	
			
					
			frontEdgeIndex =  i-1;
			frontEdgeValue = OutputSignal1.Values(TrialNumber,i-1);
		end
	end
