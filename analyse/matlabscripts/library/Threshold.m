%This function marks every moments where signal goes 
%over a specified threshold
function Threshold (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Threshold, BypassTime)
%beginAnalyseHeader
%GUIFunctionName = Threshold
%MatlabFunctionName = Threshold
%ShortDescritpion = <p>This function marks every moments where signal goes over a specified threshold</p>
%LongDescription = <p>This function marks every moments where signal goes over a specified threshold</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		for i=fromIndex:toIndex
			currentTime = (i-1)/InputSignal1.SampleFrequency;
			if((detectionAllowed == 1) && ((currentTime - detectionTime) >= BypassTime))
				if(InputSignal1.Values(TrialNumber,i) > Threshold)
