%This function computes RMS value of input signal
%using rectangle method between two markers
function RMS (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = RMS
%MatlabFunctionName = RMS
%ShortDescritpion = <p>Compute RMS value of signal input between two markers</p>
%LongDescription = <p>This function computes RMS value of input signal using retangle method between two markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		length=size(InputSignal1.Values(TrialNumber,fromIndex:toIndex));
		OutputField1.Values(TrialNumber)=sqrt( sum(InputSignal1.Values(TrialNumber,fromIndex:toIndex).^2)/length(2));
	end
