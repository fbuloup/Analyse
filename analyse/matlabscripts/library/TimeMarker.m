%Creates a marker at a chosen time. If trial duration is shorter than specified time, you can choose to mark the last trial sample.function TimeMarker (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Time, MarkLastSample)
%beginAnalyseHeader
%GUIFunctionName = TimeMarker
%MatlabFunctionName = TimeMarker
%ShortDescritpion = <p>Creates a marker at a chosen time. If trial duration is shorter than specified time, you can choose to mark the last trial sample.</p>
%LongDescription = <p>Creates a marker at a chosen time. If trial duration is shorter than specified time, you can choose to mark the last trial sample.</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 0
%FieldsUsedNumber = 0
...
%param1Component = 1
%param1Label = Time
%param1ToolTip = Time wanted
%param1RegExp = ^[+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$
...
%param2Component = 2
%param2Label = MarkLastSample
%param2ToolTip = Mark last sample
%param2RegExp = ^(true|false)$
%param2AvailableListValue = 
%param2DefaultValue = false
...
for TrialNumber = TrialsList
	...
	i = round(Time*InputSignal1.SampleFrequency+1);
	L = InputSignal1.EndCut(TrialNumber);
	
	if( L >=  i)
		NewTime = (i-1)/InputSignal1.SampleFrequency;
		OutputMarker1.Values=[OutputMarker1.Values; TrialNumber NewTime InputSignal1.Values(TrialNumber,i)];
	else
		if( MarkLastSample == true )
			NewTime = (InputSignal1.EndCut(TrialNumber) - 1)/InputSignal1.SampleFrequency;
			NewIndice = InputSignal1.EndCut(TrialNumber) - 1;
			OutputMarker1.Values=[OutputMarker1.Values; TrialNumber NewTime InputSignal1.Values(TrialNumber, NewIndice)];
		end
	end
