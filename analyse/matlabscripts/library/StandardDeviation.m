%<p>Computes the standard deviation between two markers</p>function StandardDeviation (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Standard Deviation
%MatlabFunctionName = StandardDeviation
%ShortDescritpion = <p>Standard deviation between two markers</p>
%LongDescription = <p>Computes the standard deviation between two markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		OutputField1.Values(TrialNumber)=std(InputSignal1.Values(TrialNumber,fromIndex:toIndex));
	end
