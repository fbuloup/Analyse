%Compute area under curve between two markers using trapz
function Area (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Area
%MatlabFunctionName = Area
%ShortDescritpion = <p>Compute area under curve between two markers using trapz</p>
%LongDescription = <p>Compute area under curve between two markers using trapz</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		OutputField1.Values(TrialNumber)=trapz(InputSignal1.Values(TrialNumber,fromIndex:toIndex))/(InputSignal1.SampleFrequency);
	end
