%This function computes motion range. It takes two signals
%as inputs and calculates the linear distance between the
%origin and any point of the trajectory. This trajectory is 
%formed from the two input signals.
function MotionRange (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Motion range
%MatlabFunctionName = MotionRange
%ShortDescritpion = <p>This function computes motion range</p>
%LongDescription = <p>This function computes motion range. It takes two signals as inputs and calculates the linear distance between the origin  and any point of the trajectory. This trajectory is  formed from the two input signals.</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		for i=fromIndex:toIndex
			OutputSignal1.Values(TrialNumber,i)=sqrt( (InputSignal1.Values(TrialNumber,i) - x0)^2 + (InputSignal2.Values(TrialNumber,i) - y0)^2 );
		end
	end
