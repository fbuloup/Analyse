%Curvilinear abscissa between two markersfunction CurveLength (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Curve Length
%MatlabFunctionName = CurveLength
%ShortDescritpion = <p>Curvilinear abscissa between two markers</p>
%LongDescription = <p>Curvilinear abscissa between two markers</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		A=InputSignal1.Values(TrialNumber,fromIndex:toIndex);
		B=InputSignal2.Values(TrialNumber,fromIndex:toIndex);
		L=toIndex - fromIndex + 1;
		
		OutputField1.Values(TrialNumber)=0;
		
		for i=1:L-1
			x=sqrt((A(i+1)-A(i))^2+(B(i+1)-B(i))^2);
			OutputField1.Values(TrialNumber)=OutputField1.Values(TrialNumber)+x;
		end
