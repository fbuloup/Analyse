...
function RiseStartFromMarker (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, DeltaT, DevPercent)
%beginAnalyseHeader
%GUIFunctionName = RiseStartFromMarker
%MatlabFunctionName = RiseStartFromMarker
%ShortDescritpion = <p>short Description</p>
...
for TrialNumber = TrialsList
	...
	index = find(InputMarker1.Values(:,1)==TrialNumber);
	
	if(~isempty(index))
	
		L=length(InputSignal1.Values(TrialNumber,:));
		n=DevPercent/100;
		Deltai=DeltaT*InputSignal1.SampleFrequency;
		
		moy=mean(InputSignal1.Values(TrialNumber,1:Deltai));
		ecart=std(InputSignal1.Values(TrialNumber,1:Deltai));
		
		i=round(InputMarker1.Values(index(1),2)*InputSignal1.SampleFrequency+1);
		while ((InputSignal1.Values(TrialNumber,i)>=moy+ecart*n)&(i>1))
			i=i-1;
		end
		if (InputSignal1.Values(TrialNumber,i)<moy+ecart*n)
