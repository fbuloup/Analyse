...
function XStandardization (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Rate)
%beginAnalyseHeader
%GUIFunctionName = XStandardization
%MatlabFunctionName = XStandardization
%ShortDescritpion = <p>X axis standardization</p>
%LongDescription = <p>X axis standardization</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 0
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if Rate>1
		OutputSignal1.NbSamples(TrialNumber)=ceil(Rate*InputSignal1.NbSamples(TrialNumber))-Rate+1;
	else
		OutputSignal1.NbSamples(TrialNumber)=ceil(Rate*InputSignal1.NbSamples(TrialNumber));
	end
	L=InputSignal1.NbSamples(TrialNumber);
	LL=OutputSignal1.NbSamples(TrialNumber);
	
	OutputSignal1.Values = [OutputSignal1.Values; zeros(1,LL)];
	OutputSignal1.SampleFrequency = InputSignal1.SampleFrequency * Rate;
	
	result = interp1(1:L,InputSignal1.Values(TrialNumber,:),1:1/Rate:L,'spline');
	OutputSignal1.Values(TrialNumber,1:length(result)) = result;
