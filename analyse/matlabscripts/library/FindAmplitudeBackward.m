%This function finds an amplitude, specified as parameter, starting from the second input marker and going backward to the first one.function FindAmplitudeBackward (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber, Amplitude)
%beginAnalyseHeader
%GUIFunctionName = Find Amplitude Backward
%MatlabFunctionName = FindAmplitudeBackward
%ShortDescritpion = <p>This function finds an amplitude, specified as parameter, starting from the second input marker and going backward to the first one.</p>
%LongDescription = <p>This function finds an amplitude, specified as parameter, starting from the second input marker and going backward to the first one.</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		i = toIndex - 1;
		previousValue =  InputSignal1.Values(TrialNumber, toIndex);
		found = 0;
		while(i >= fromIndex)
			...
			if(found == 1)
				...
				if(dCurrent > dPrevious) 
					OutputMarker1.Values = [OutputMarker1.Values; TrialNumber i/InputSignal1.SampleFrequency previousValue];
					break;
				else
