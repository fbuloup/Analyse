%Creates a marker at the absolute maximum of the signal between two markers
function Maximum (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Maximum
%MatlabFunctionName = Maximum
%ShortDescritpion = <p>Maximum between two markers</p>
%LongDescription = <p>Creates a marker at the absolute maximum of the signal between two markers</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		[maxValue, maxIndex] = max(InputSignal1.Values(TrialNumber,fromIndex:toIndex));
		OutputMarker1.Values=[OutputMarker1.Values; TrialNumber (fromIndex + maxIndex - 2)/InputSignal1.SampleFrequency maxValue];
	end
