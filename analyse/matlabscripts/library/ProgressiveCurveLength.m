%This function computes the length of the path at each of its points. This path is formed by the two input signals.function ProgressiveCurveLength (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Progressive Curve Length
%MatlabFunctionName = ProgressiveCurveLength
%ShortDescritpion = <p>This function computes the length of the path at each of its points. This path is formed by the two input signals.</p>
%LongDescription = <p>This function computes the length of the path at each of its points. This path is formed by the two input signals.</p>
%SignalsUsedNumber = 2
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		A=InputSignal1.Values(TrialNumber,:);
		B=InputSignal2.Values(TrialNumber,:);
		L=InputSignal1.NbSamples(TrialNumber);
		
		for i=fromIndex:toIndex-1
			x=sqrt((A(i+1)-A(i))^2+(B(i+1)-B(i))^2);
			OutputSignal1.Values(TrialNumber,i+1)=OutputSignal1.Values(TrialNumber,i)+x;
		end
	end
