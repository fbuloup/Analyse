%Creates a marker at each extremum 
%(maximum or mininmum) between two markers.function Extrema (TrialsList , signalsNamesList, markersNamesList, fieldsNamesList, signalsNamesSuffix, newMarkersNamesList, newFieldsNamesList, signalsModifiedNumber)
%beginAnalyseHeader
%GUIFunctionName = Extrema
%MatlabFunctionName = Extrema
%ShortDescritpion = <p>Extrema between two markers</p>
%LongDescription = <p>Creates a marker at each extremum (maximum or mininmum) between two markers.</p>
%SignalsUsedNumber = 1
%MarkersUsedNumber = 2
%FieldsUsedNumber = 0
%SignalsCreatedNumber = 0
%MarkersCreatedNumber = 2
...
for TrialNumber = TrialsList
	...
	if(~isempty(fromTrialIndices) && ~isempty(toTrialIndices))
		...
		deriv=diff(InputSignal1.Values(TrialNumber,fromIndex:toIndex));
		mySign = sign(deriv);
		
		minindex=find(diff(mySign)==2);
		maxindex=find(diff(mySign)==-2);
		
		minvalue=InputSignal1.Values(TrialNumber,minindex)
		maxvalue=InputSignal1.Values(TrialNumber,maxindex)
	
		for i=1:length(minindex)
			OutputMarker1.Values=[OutputMarker1.Values; TrialNumber (fromIndex + minindex(i) - 1)/InputSignal1.SampleFrequency minvalue(i)];
		end
		for i=1:length(maxindex)
			OutputMarker2.Values=[OutputMarker2.Values; TrialNumber (fromIndex + maxindex(i) - 1)/InputSignal1.SampleFrequency maxvalue(i)];
		end
	end
