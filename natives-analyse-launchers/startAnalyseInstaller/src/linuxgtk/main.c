...
int main (int argc, char *argv[])
{
  ...
  strcat(execCmd,"/LINUXAnalyseInstallerLauncher.sh");
  ...
  if(javaExist) {
    ...
    if(status == 1) {
        ...
        printf("Launch analyse\n");
        g_type_init();
        struct stat statInfo;
        stat("/home/frank/startAnalyseCmde.tmp", &statInfo);
        printf("File size : %d\n", statInfo.st_size);

        char* homeDir = getenv("HOME");
        strcat(homeDir, "/startAnalyseCmde.tmp");
        printf("Home path : %s\n", homeDir);

        GFile* tmpFile = g_file_new_for_path(homeDir);
        GFileInputStream* inputStream = g_file_read(tmpFile,NULL,NULL);
        gsize* readSize;
        char* buffer = (char *)calloc(PATH_MAX, sizeof(char));;
        g_input_stream_read_all(inputStream,buffer,statInfo.st_size,readSize,NULL,NULL);
        strcat(buffer, "startAnalyse");
        printf("Cmd : %s\n", buffer);
        //launch Analyse
        system(buffer);
        //exec("/bin/sh","sh","-c",buffer,NULL);
    }*/
    printf("Exit\n");
    return 0;
  }
