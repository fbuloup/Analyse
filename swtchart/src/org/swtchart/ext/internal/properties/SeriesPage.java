...
public class SeriesPage extends AbstractSelectorPage {
    ...
    public SeriesPage(InteractiveChart chart, PropertiesResources resources,
        ...
        lineWidths = new int[series.length];
    ...
    protected void selectInitialValues() {
        for (int i = 0; i < series.length; i++) {
            ...
            if (series[i] instanceof ILineSeries) {
                ...
                lineWidths[i] = ((ILineSeries) series[i]).getLineWidth();
            ...
            } else if (series[i] instanceof IEventSeries) {
            	lineColors[i] = ((IEventSeries) series[i]).getLineColor().getRGB();
            	lineStyles[i] = ((IEventSeries) series[i]).getLineStyle();
                lineWidths[i] = ((IEventSeries) series[i]).getLineWidth();
            	symbolColors[i] = ((IEventSeries) series[i]).getSymbolColor().getRGB();
		        symbolTypes[i] = ((IEventSeries) series[i]).getSymbolType();
		        symbolSizes[i] = ((IEventSeries) series[i]).getSymbolSize();
            }
    ...
    protected void updateControlSelections() {
        ...
        if (series[selectedIndex] instanceof ILineSeries) {
            ...
            lineWidthSpinner.setSelection(lineWidths[selectedIndex]);
        ...
        } else if (series[selectedIndex] instanceof IEventSeries) {
        	lineStyleCombo.setText(lineStyles[selectedIndex].label);
            lineStyleCombo.setEnabled(true);
            lineWidthSpinner.setSelection(lineWidths[selectedIndex]);
            lineColorButton.setColorValue(lineColors[selectedIndex]);
    ...
    private void addLineSeriesGroup(Composite parent) {
        ...
        createLabelControl(lineSeriesGroup, "Line width:");
        lineWidthSpinner = createSpinnerControl(lineSeriesGroup, 1, 10);
        lineWidthSpinner.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                String value = lineWidthSpinner.getText();
                lineWidths[selectedIndex] = Integer.valueOf(value);
            }
        });
    ...
    public void apply() {
        for (int i = 0; i < series.length; i++) {
            ...
            if (series[i] instanceof ILineSeries) {
                ...
                ((ILineSeries) series[i]).setLineWidth(lineWidths[i]);
            	...
            	Color lineColor = new Color(Display.getDefault(), lineColors[i]);
                ((IEventSeries) series[i]).setLineColor(lineColor);
                final String lineColorKey = SERIES_LINE_COLOR
                        + series[i].getId();
                if (resources.getColor(lineColorKey) == null) {
                    series[i].addDisposeListener(new IDisposeListener() {
                ...
                Color symbolColor = new Color(Display.getDefault(),
                        symbolColors[i]);
                ((IEventSeries) series[i]).setSymbolColor(symbolColor);
                final String symbolColorKey = SERIES_SYMBOL_COLOR
                        + series[i].getId();
                if (resources.getColor(symbolColorKey) == null) {
                    series[i].addDisposeListener(new IDisposeListener() {
                ...
                ((IEventSeries) series[i]).setLineStyle(lineStyles[i]);
                ((IEventSeries) series[i]).setLineWidth(lineWidths[i]);
                ((IEventSeries) series[i]).setSymbolType(symbolTypes[i]);
                ((IEventSeries) series[i]).setSymbolSize(symbolSizes[i]);
            }
