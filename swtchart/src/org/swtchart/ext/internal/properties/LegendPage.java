...
public class LegendPage extends AbstractPage {
    ...
    private void addLegendPanel(Composite parent) {
        ...
        createLabelControl(group, "Legend postion:");
        legendPositionCombo = new Combo(group, SWT.BORDER | SWT.SINGLE | SWT.READ_ONLY);
        legendPositionCombo.setItems(new String[]{"Top","Right","Bottom","Left"});
        legendPositionCombo.setLayoutData(new GridData(SWT.FILL, SWT.FILL,true,false));
        if(chart.getLegend().getPosition() == SWT.TOP) legendPositionCombo.select(0);
        if(chart.getLegend().getPosition() == SWT.RIGHT) legendPositionCombo.select(1);
        if(chart.getLegend().getPosition() == SWT.BOTTOM) legendPositionCombo.select(2);
        if(chart.getLegend().getPosition() == SWT.LEFT) legendPositionCombo.select(3);
        legendPositionCombo.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				String position = legendPositionCombo.getText();
				if(position.equals("Top")) chart.getLegend().setPosition(SWT.TOP);
				if(position.equals("Right")) chart.getLegend().setPosition(SWT.RIGHT);
				if(position.equals("Bottom")) chart.getLegend().setPosition(SWT.BOTTOM);
				if(position.equals("Left")) chart.getLegend().setPosition(SWT.LEFT);
			}
		});
