...
public class Legend extends Composite implements ILegend, PaintListener {
    ...
    public Legend(Chart chart, int style) {
        super(chart, style | SWT.NO_FOCUS);
        this.chart = chart;
    ...
    public void updateLayoutData() {
        ...
        String[] seriesID = new String[seriesArray.length];
        for (int i = 0; i < seriesArray.length; i++) seriesID[i] = seriesArray[i].getId();
        ...
        if (position == SWT.RIGHT || position == SWT.LEFT) {
            ...
            for (int j = 0; j < seriesID.length; j++) {
                int textWidth = Util.getExtentInGC(getFont(), seriesID[j]).x;
                ...
                cellBounds.put(seriesID[j], new Rectangle(maxCellWidth
            ...
            for (int j = 0; j < seriesID.length; j++) {
                ...
                cellBounds.put(seriesID[j], new Rectangle(xPosition
    ...
    protected void drawSymbol(GC gc, Series series, Rectangle r) {
        ...
        if (series instanceof ILineSeries) {
        ...
        } else if (series instanceof IEventSeries) {
        	 // draw plot line
            gc.setForeground(((IEventSeries) series).getLineColor());
            gc.setLineWidth(LINE_WIDTH);
            int lineStyle = Util.getIndexDefinedInSWT(((IEventSeries) series)
                    .getLineStyle());
            int x = r.x;
            int y = r.y + r.height / 2;
            if (lineStyle != SWT.NONE) {
            ...
            // draw series symbol
            Color color = ((IEventSeries) series).getSymbolColor();
            Color[] colors = ((IEventSeries) series).getSymbolColors();
            if (colors != null && colors.length > 0) {
                color = colors[0];
            }
            ((EventSeries) series).drawSeriesSymbol(gc, x + SYMBOL_WIDTH / 2, y,
                    color);
        }
    }
