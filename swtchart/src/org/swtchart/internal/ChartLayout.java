...
public class ChartLayout extends Layout {
    ...
    private void layoutRightAxis(Rectangle r, AxisLayoutData layoutData) {
        ...
        int width = layoutData.titleLayoutdata.widthHint;
        int height = r.height - bottomAxisHeight - topAxisHeight - titleHeight - MARGIN * 2;
        height = height - ((titleHeight == 0) ? 0 : PADDING); 
        height = height - (legendPosition == SWT.TOP || legendPosition == SWT.BOTTOM ? legendHeight + (legendHeight == 0 ? 0 : PADDING) : 0);
        int x = r.width - width - rightAxisOffset - (legendPosition == SWT.RIGHT ?legendWidth:0) - MARGIN;
        x = x - (legendPosition == SWT.RIGHT ? (legendWidth == 0 ? 0 : PADDING) : 0);
        int y = titleHeight + topAxisHeight + MARGIN + ((titleHeight == 0) ? 0 : PADDING); 
        y = y + (legendPosition == SWT.TOP ? legendHeight + (legendHeight == 0 ? 0 : PADDING) : 0);
