...
public class Axis implements IAxis {
    ...
    public void adjustRange(boolean update) {
        ...
        boolean hasOnlyEventSeries = true;
        for (ISeries series : chart.getSeriesSet().getSeries()) {
        	hasOnlyEventSeries = hasOnlyEventSeries && (series instanceof EventSeries);
        }
        ...
        for (ISeries series : chart.getSeriesSet().getSeries()) {
            ...
            if(series instanceof EventSeries && !hasOnlyEventSeries) {
            	
            	if(((EventSeries)series).isShowHeightAsInfinite()) continue;
            	
            	double[] hSeries = ((EventSeries)series).getHSeries();
            	boolean hasOnlyInfiniteAmplitude = true;
            	for (int i = 0; i < hSeries.length; i++) {
    				if(!Double.isInfinite(hSeries[i])) {
    					hasOnlyInfiniteAmplitude = false;
    					break;
    				}
    			}
            	
            	if(hasOnlyInfiniteAmplitude) continue;
            }
